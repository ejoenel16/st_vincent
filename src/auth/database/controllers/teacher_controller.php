...
if (!$option) {
    ...
    echo 1;
} else if ($option == "getActiveTeachersList") {
...
} else if ($option == "get_teachers_by_subject_id") {
...
} else if ($option == "get_total_teachers") {
    $db = new Database();

    $q = '
            SELECT 
                COUNT(id) as totalTeachers
            FROM
                tbl_teachers
        ';
...
} else if ($option == "get_active_teachers") {
    $db = new Database();
