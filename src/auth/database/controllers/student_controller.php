    ...
    $q = '
        ...
        WHERE
            A.id in ( 
                        ...
                        WHERE 
                            status = "enrolled" group by student_id 
                    )
        ';
...
// CUSTOMIZED FUNCTIONS HERE
    ...
    $sf9_dir = "src/public/students_sf9/u_" . $rand_uniq .  $lrn;
    move_uploaded_file(
        $sf9,
        "../../public/students_sf9/u_" . $rand_uniq .  $lrn
    );
    ...
    $sf10_dir = "src/public/students_sf10/u_" . $rand_uniq .  $lrn;
    move_uploaded_file(
        $sf10,
        "../../public/students_sf10/u_" . $rand_uniq .  $lrn
    );
    ...
    $nso_dir = "src/public/students_nso/u_" . $rand_uniq .  $lrn;
    move_uploaded_file(
        $nso,
        "../../public/students_nso/u_" . $rand_uniq .  $lrn
    );
    ...
    $gmc_dir = "src/public/students_gmc/u_" . $rand_uniq . $lrn;
    move_uploaded_file(
        $gmc,
        "../../public/students_gmc/u_" . $rand_uniq . $lrn
    );
    ...
    $photo_dir = "src/public/students_photo/u_" . $rand_uniq . $lrn;

    move_uploaded_file(
        $photo,
        "../../public/students_photo/u_" . $rand_uniq . $lrn
    );
    ...
    if ($res == 1) {
        ...
        $_EMAIL_ACKNOWLEDGE_EMAIL_SUBJECT = "Acknowledgement Email";
        $_EMAIL_ACKNOWLEDGE_EMAIL_BODY = "This is to acknowledge your application! Expect another email once you get adimitted.";
        $_EMAILTO = $_POST['email'];

        echo send_email($_EMAILTO, $_EMAIL_ACKNOWLEDGE_EMAIL_SUBJECT, $_EMAIL_ACKNOWLEDGE_EMAIL_BODY);
    } else {
        echo 0;
    }
    ...
    echo $db->update($q, $d);
} else if ($option == "archive_student_application") {
    ...
    $STUDENT_EMAIL = $db->read($q_get_email, $d_get_email)[0]['email'];
    ...
    if ($res == 1) {
        ...
        $_EMAIL_ACKNOWLEDGE_EMAIL_SUBJECT = "Welcome Student!";
        $_EMAIL_ACKNOWLEDGE_EMAIL_BODY = "We are glad to welcome you in SVIT! <br>
        You can now access your account using your email and your current password is : 
        <b>$GENERATED_PASSWORD</b><br>Kindly change your password right away.";

         send_email(
            $STUDENT_EMAIL,
            $_EMAIL_ACKNOWLEDGE_EMAIL_SUBJECT,
            $_EMAIL_ACKNOWLEDGE_EMAIL_BODY
        );
    } 
    echo 1;
...
} else if ($option == "get_total_students") {
    $db = new Database();
...
} else if ($option == "get_total_preEnrolled") {
    $db = new Database();

    $q = '
            SELECT 
                COUNT(id) as totalPreEnrolled
            FROM
                tbl_student_applications
            WHERE
                status = "pending"
