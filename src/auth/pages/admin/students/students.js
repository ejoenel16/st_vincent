...
$(document).ready(function () {
  ...
  // BUTTONS
  $("#btn_edit_student_profile_save_changes")
    .unbind("click")
    .click(function () {
      payload = {
  ...
  $("#btn_edit_student_profile_change_photo")
    .unbind("click")
    .click(function () {
      $("#md_confirm_student_change_photo").modal("show");
      $("#md_confirm_student_change_photo_btn_confirm")
        .attr("lrn", lrn)
        .unbind("click")
        .click(function () {
          payload = {
            ...
            application_no:
              data_collection_student_information[lrn].application_no,
          };
  ...
  $("#student_btn_changepass_save_changes")
    .unbind("click")
    .click(function () {
      payload = {
  ...
  _GET(con_student, payload, (cb) => {
    ...
    $.each(cb, (k, v) => {
      output +=
        ...
        `</td>
                    <td class="text-center">` +
        check_if_enrolled(v.lrn) +
        `</td>
                    <td class="text-center">` +
        v.gender +
        `</td>
                    <td class="text-center">
                        <button class="students_edit btn p-1" lrn="` +
        v.lrn +
        `">
                            <i class="fa-solid fa-pencil"></i>
                        </button>
                        <button class="students_archive btn p-1" lrn="` +
        v.lrn +
        `">
    ...
    $(".students_archive")
      ...
      .click(function () {
        ...
        $("#div__confirm_archive_student").modal("show");

        $("#btn__confirm_archive_student")
          .unbind("click")
          .click(function () {
            payload = {
              request_type: "archive_student_application",
              lrn: lrn,
            };

            _POST(con_student, payload, (cb) => {
              if (cb == 1) {
                show_toast("Archive Student", "Archive student successful");
                s_get_students();
              } else {
                show_toast(
                  "Contact Administrator",
...
function check_if_enrolled(lrn) {
  if (lrn in data_collections_enrolled_students) {
    ...
    return '<span class="p-1" style="display:block; width:100%; background:#FFF8EB;color:#EEA23E;border-radius:5px">Enrolled</span>';
  }
}
...
function students_view_profile(lrn) {
  student_gender = "";
  ...
  // HEADER
  $("#student_header_name").text(
    data_collection_student_information[lrn].lastname +
      ", " +
      data_collection_student_information[lrn].firstname +
      " " +
      singlify_word(data_collection_student_information[lrn].middlename)
  );
  $("#student_header_email").text(
    data_collection_student_information[lrn].email
  );
  $("#student_header_lrn").text(data_collection_student_information[lrn].lrn);
  $("#student_header_grade_level").text(
    data_collection_student_information[lrn].grade_level
  );
  $("#student_header_section").html(check_if_enrolled(lrn));
  $("#student_header_strand").text(student_strand);
  $("#student_header_semester").text(
    data_collection_student_information[lrn].semester
  );
  $("#student_header_sex").text(
    data_collection_student_information[lrn].gender
  );

  student_gender = data_collection_student_information[lrn].gender;

  // INFO
  $("#student_info_lname").val(
    data_collection_student_information[lrn].lastname
  );
  $("#student_info_fname").val(
    data_collection_student_information[lrn].firstname
  );
  $("#student_info_mname").val(
    data_collection_student_information[lrn].middlename
  );
  $("#student_info_dob").val(
    data_collection_student_information[lrn].birthdate
  );
  ...
  $("#student_info_contact_no").val(
    data_collection_student_information[lrn].mobile_no
  );
  $("#student_info_add_street").val(
    data_collection_student_information[lrn].add_st
  );
  $("#student_info_add_city").val(
    data_collection_student_information[lrn].add_city
  );
  $("#student_info_add_state").val(
    data_collection_student_information[lrn].add_state
  );
  $("#student_info_add_zipcode").val(
    data_collection_student_information[lrn].add_zipcode
  );
  $("#student_info_e_fname").val(
    data_collection_student_information[lrn].guardian_name
  );
  $("#student_info_e_contact").val(
    data_collection_student_information[lrn].guardian_no
  );
