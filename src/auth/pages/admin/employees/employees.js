...
data_collections_array_subjects_to_edit = {};
...
$(document).ready(function () {
    ...
    $('#employee_md_add_subject_to_teacher_confirm').click(function () {

        const addSubjectFrom = $(this).attr('from')
        const subjectId = $('#md_sel_add_to_teacher').val()
        const subjectName = $('#md_sel_add_to_teacher option:selected').text()

        subjectId in teacherSubjectList ? '' : teacherSubjectList[subjectId] = {
            id: subjectId,
            subject: subjectName
        }

        switch (addSubjectFrom) {
            case 'editTeacher':
                loadTeacherWriteSubjects()
                break;
            case 'registerTeacher':
                registerTeacherWriteSubjects()
                break;
        }
    })
    ...
    $('#a_btn_register_teacher').click(function () {
        ...
        if (e_register_employees_validate_inputs()) {
            ...
            _POST(API_URL_TEACHERS, payload, (cb) => {
                if (cb == 1) {
...
function e_load_subjects() {
    ...
    _GET(sys_con_subjects, payload, (cb) => {
        ...
        $('#md_sel_add_to_teacher').html(output)
...
function e_draw_teachers_subjects_edit() {

    output = `
        <button class="btn btn_edit_subject_to_teacher"  style="font-size:9pt;">
            <i class="fa-solid fa-plus"></i>
        </button>
    `;

    $.each(data_collections_array_subjects_to_edit, (k, v) => {
        output += `
        <span class="d-inline-block my-1 px-3 py-1 rounded" style="background:#F5FAFF; color:#0A1881">
            ...
            <a class="ms-2 py-0 a__edit_teacher_deleteSubject" subject_id=`+ k + `>
    ...
    $('#div_info_teacher_subjects').html(output)

    $('.a__edit_teacher_deleteSubject').unbind('click').click(function () {
        $.each(data_collections_array_subjects_to_edit, (k, v) => {
            if (k == $(this).attr('subject_id')) {
                delete data_collections_array_subjects_to_edit[k]
            }
        })
        e_draw_teachers_subjects_edit()
    })

    $('.btn_edit_subject_to_teacher').unbind('click').click(function () {
        $('#employee_md_add_subject_to_teacher').modal('show')
        $('#employee_md_add_subject_to_teacher_confirm').hide();
        $('#employee_md_edit_subject_to_teacher_confirm').show();
    })

    $('#employee_md_edit_subject_to_teacher_confirm').unbind('click').click(function () {
        if (data_collections_array_subjects_to_edit.length === 0) {
            data_collections_array_subjects_to_edit = data_collections_array_subjects_to_add
        }

        if (!($('#md_sel_add_to_teacher').val() in data_collections_array_subjects_to_edit)) {
            data_collections_array_subjects_to_edit['e' + $('#md_sel_add_to_teacher').val()] = {
...
function e_draw_teachers_subjects() {
    ...
    output = `
        <button class="btn" id="teacherRegisterAddSubjectBtn" style="font-size:9pt;">
            <i class="fa-solid fa-plus"></i>
        </button>
    `;
    ...
    $.each(data_collections_array_subjects_to_add, (k, v) => {
        ...
        <span class="d-inline-block my-1 px-3 py-1 rounded" style="background:#F5FAFF; color:#0A1881">
            <span style="font-size:9pt;">`+ v.subject + `</span>
            <a class="ms-2 py-0 a__add_teacher_deleteSubject" subject_id=`+ k + `>
                <i style="font-size:7.5pt;" class=" fa-solid fa-x"></i>
            </a>
        </span>?
    `;
    ...
    $('.a__add_teacher_deleteSubject').unbind('click').click(function () {
        $.each(data_collections_array_subjects_to_add, (k, v) => {
            if (k == $(this).attr('subject_id')) {
                delete data_collections_array_subjects_to_add[k]
            }
        })
        e_draw_teachers_subjects()
    })

    $('#teacherRegisterAddSubjectBtn').click(function () {
        $('#employee_md_add_subject_to_teacher').modal('show')
    })
}
...
function e_load_teachers() {
    ...
    _GET(con_teachers, payload, (cb) => {
        ...
        $('#tbl_admin_teachers tbody').html(output)

        $('.employee_btn_edit').unbind('click').click(function () {

            const teacherId = $(this).attr('teacher_id')
            ...
            $('#btn_edit_teacher_profile_save_changes')
                .attr('teacher_id', teacherId)
            ...
            $('#btn_changepass_profile_save_changes')
                .attr('teacher_id', teacherId)
                .unbind('click').click(function () {

                    $('#employee_md_tp_changepassword').modal('show');
                    $('#employee_md_tp_changepassword_btn_confirm').attr('teacher_id', teacherId).unbind('click').click(function () {
            ...
            e_load_teacher_information(teacherId)
        })
...
const loadTeacherWriteSubjects = () => {
    ...
    output = `
        <button class="btn" id="btnAddSubjectToTeacher" style="font-size:9pt;">
            <i class="fa-solid fa-plus"></i>
        </button>
    `;
    ...
    $.each(teacherSubjectList, (k, v) => {
        output += `
            <span class="d-inline-block my-1 px-3 py-1 rounded" style="background:#F5FAFF; color:#0A1881">
                <span style="font-size:9pt;"> `+ v.subject + ` </span>
                <a class="ms-2 py-0 a__edit_teacher_deleteSubject" subject_id=`+ v.id + `>
                    <i style="font-size:7.5pt;" class=" fa-solid fa-x"></i>
                </a>
            </span>
        `;
    ...
    $('.a__edit_teacher_deleteSubject').unbind('click').click(function () {
        const subjectIdToDelete = $(this).attr('subject_id')
        delete teacherSubjectList[subjectIdToDelete]
        loadTeacherWriteSubjects()
    })

    $('#btnAddSubjectToTeacher').unbind('click').click(function () {
        $('#employee_md_add_subject_to_teacher').modal('show')
        $('#employee_md_add_subject_to_teacher_confirm').attr('from', 'editTeacher')
    })
}

const registerTeacherWriteSubjects = () => {
    output = `
        <button class="btn" id="btn_add_subject_to_teacher" style="font-size:9pt;">
            <i class="fa-solid fa-plus"></i>
        </button>
    `;
    ...
    $.each(teacherSubjectList, (k, v) => {
        output += `
            <span class="d-inline-block my-1 px-3 py-1 rounded" style="background:#F5FAFF; color:#0A1881">
                <span style="font-size:9pt;"> `+ v.subject + ` </span>
                <a class="ms-2 py-0 a__add_teacher_deleteSubject" subject_id=`+ v.id + `>
                    <i style="font-size:7.5pt;" class=" fa-solid fa-x"></i>
                </a>
            </span>
        `;
    ...
    $('#btn_add_subject_to_teacher').unbind('click')
        .click(function () {
            $('#employee_md_add_subject_to_teacher').modal('show')
            $('#employee_md_add_subject_to_teacher_confirm').attr('from', 'registerTeacher')
        })

    $('.a__add_teacher_deleteSubject').unbind('click')
        .click(function () {
            const subjectIdToDelete = $(this).attr('subject_id')
            delete teacherSubjectList[subjectIdToDelete]
            registerTeacherWriteSubjects()
...
function e_load_staffs() {
    ...
    _GET(con_staffs, payload, (cb) => {
        ...
        $('#tbl_admin_staffs tbody').html(output)
...
const clearRegisterTeacherFields = () =>{
    $('#tr_info_lname').val('')
    $('#tr_info_fname').val('')
    $('#tr_info_mname').val('')
    $('#tr_info_email').val('')
    $('#tr_info_contact_no').val('')
    $('#tr_info_street').val('')
    $('#tr_info_city').val('')
    $('#tr_info_state').val('')
    $('#tr_info_zipcode').val('')
    $('#tr_info_e_fname').val('')
    $('#tr_info_e_contact_no').val('')
    $('#tr_info_e_relationship').val('')
    teacherSubjectList = {};
}