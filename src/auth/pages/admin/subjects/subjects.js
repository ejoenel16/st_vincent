...
$("#subjects_sem_1").click(function () {
  $("#strand_txt_grade_level").text(
    $("#strand_txt_grade_level").text().replace("Inactive", "Active")
  );
...
$("#subjects_sem_2").click(function () {
  $("#strand_txt_grade_level").text(
    $("#strand_txt_grade_level").text().replace("Active", "Inactive")
  );
...
function admin_get_subjects(id, level = null) {
  if (level == null) {
    ...
    _GET(sys_con_subjects, payload, (cb) => {
      ...
      $.each(cb, (k, v) => {
        output +=
          ...
          `
                    ...
                    <td class="text-end">
                        <button class="strands_btn_subject_delete btn p-1" subject_id="` +
          v.id +
          `">
      ...
      $(".strands_btn_subject_delete")
        .unbind("click")
        .click(function () {
          $("#subjects_md_remove_subject").modal("show");
          $("#btn_subjects_md_removeconfirm")
            .attr("subject_id", $(this).attr("subject_id"))
            .unbind("click")
            .click(function () {
              payload = {
              ...
              _POST(sys_con_subjects, payload, (cb) => {
                if (cb == 1) {
                  show_toast(
                    "Remove Subject",
                    "Subject has been removed from the strand"
                  );
  ...
  _GET(sys_con_strands, payload, (cb) => {
    ...
    $.each(cb, (k, v) => {
      output +=
        ...
        `</td>
                    <td class="text-end">
                        <button class="strands_btn_edit btn p-1" strand_id="` +
        v.id +
        `">
                            <i class="fa-solid fa-pencil"></i>
                        </button>
                        <button class="strands_btn_add_subjects btn p-1" strand_id="` +
        v.id +
        `">
                            <i class="fa-solid fa-gear"></i>
                        </button>
                        <button class="strands_btn_delete btn p-1" strand_id="` +
        v.id +
        `">
                            <i class="fa-regular fa-trash-can"></i>
                        </button>
    ...
    $(".strands_btn_edit")
      ...
      .click(function () {
        ...
        $("#strands_md_txt_grade_level").val(
          data_collections_strands[$(this).attr("strand_id")].grade_level
        );
        $("#strands_md_txt_strand").val(
          data_collections_strands[$(this).attr("strand_id")].strand
        );
        $("#strands_btn_confirm_edit")
          .attr("strand_id", $(this).attr("strand_id"))
          .unbind("click")
          .click(function () {
            payload = {
            ...
            _POST(sys_con_strands, payload, (cb) => {
              if (cb == 1) {
                show_toast(
                  "Edit Strand",
                  "Strand has been updated successfully."
                );
                admin_get_strands();
              } else {
                show_toast(
          ...
          .click(function () {
            ...
            _POST(sys_con_strands, payload, (cb) => {
              if (cb == 1) {
                show_toast(
                  "Delete Strand",
                  "Strand has been deleted successfully."
                );
                admin_get_strands();
              } else {
                show_toast(
    ...
    $(".strands_btn_add_subjects")
      ...
      .click(function () {
        ...
        if (
          data_collections_strands[$(this).attr("strand_id")].grade_level < 11
        ) {
          $("#strand_txt_grade_level").text(
            "Grade " +
              data_collections_strands[$(this).attr("strand_id")].grade_level
