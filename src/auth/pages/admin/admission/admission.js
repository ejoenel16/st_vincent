...
$(document).ready(function () {
  ...
  $(".a_approve_requirement").click(function (e) {
    ...
    if (!$(this).prop("checked")) {
      $("#btn_confirm_disapprove_requirement")
        .attr("attachment_id", $(this).attr("attachment_id"))
        .attr("document_name", $(this).attr("id"));
      ...
      $("#btn_confirm_approve_requirement")
        .attr("attachment_id", $(this).attr("attachment_id"))
        .attr("document_name", $(this).attr("id"));
  ...
  _GET(con_student, payload, (cb) => {
    ...
    $.each(cb, (k, v) => {
      if (v.status == "pending") {
        badge =
          `
                    <button class="btn p-1 a_edit" lrn="` +
          v.lrn +
          `"><i class="fa-solid fa-eye"></i></button>
                    <button class="btn p-1 a_cancel" lrn="` +
          v.lrn +
    ...
    $(".a_cancel")
      ...
      .click(function () {
        ...
        $("#btn_cancel_application_confirm")
          .unbind("click")
          .click(function () {
            payload = {
            ...
            _POST(con_student, payload, (cb) => {
              show_toast(
                ...
                "Student application has been cancelled successfully."
              );
              a_get_applications();
            });
          });
...
function a_load_application(lrn) {
  ...
  _GET(con_student, payload, (cb) => {
    ...
    $.each(cb.application_information, (k, v) => {
      ...
      if (
        v.application_status == "admitted" ||
        v.application_status == "cancelled"
      ) {
    ...
    $.each(cb.attachments, (k, v) => {
      output +=
        `
                <li>
                    <a class="btn rounded" download="SA-` +
        lrn +
        `_` +
        v.document_name +
        `" href="` +
    ...
    aRequirementIsValid = false;

    $("#a_btn_admit_student").click(function () {
      $(".a_requirement_checklist").each(function (k, v) {
        if ($(this).is(":checked")) {
          aRequirementIsValid = true;
        } else {
          aRequirementIsValid = false;
        }
      });
      ...
      $("#btn_confirm_admit_student")
        .unbind("click")
        .click(function () {
          if (aRequirementIsValid) {
            payload = {
            ...
            _POST(con_student, payload, (cb) => {
              if (cb == 1) {
                show_toast(
                  "Admit Student",
                  "Student has been admitted successfully"
                );
                a_get_applications();
              } else {
            ...
            show_toast(
              "Admission Failed",
              "You can't approve this application, one or more requirements are not met",
              "error"
            );
          }
        });
