    ...
    $.each(cb, (k, v) => {
      output +=
        ...
        `</td>
                    <td>
                        <button 
                            class="btn_get_section_students btn p-1" 
                            subject_id="` +
        v.subject_id +
        `"
                            grade_level="` +
...
function tm_load_section_students(section) {
  $("#sag_subject").text(
    tm_dc_information[$(section).attr("section_id")].subject
  );
  ...
  _GET(con_teacher, payload, (cb) => {
      ...
      output +=
        ...
        `</td>
                    ...
                    <td class="text-center" style="font-size:9pt; font-weight:600">
                        <button class="edit_student_grade btn p-1" lrn="` +
        student.lrn +
        `">
    ...
    $(".edit_student_grade")
      ...
      .click(function () {
        ...
        if (temp_grade_collection[lrn]) {
          $("#sag_grade_1").val(
            temp_grade_collection[$(this).attr("lrn")].grade_1
          );
          $("#sag_grade_2").val(
            temp_grade_collection[$(this).attr("lrn")].grade_2
          );
          $("#sag_grade_3").val(
            temp_grade_collection[$(this).attr("lrn")].grade_3
          );
          $("#sag_grade_4").val(
            temp_grade_collection[$(this).attr("lrn")].grade_4
          );
          $("#sag_grade_f").val(
            temp_grade_collection[$(this).attr("lrn")].grade_5
          );
        } else {
          $("#sag_grade_1").val(0);
          $("#sag_grade_2").val(0);
    ...
    $(".grade_input").keyup(function (k) {
      try {
        ...
        final_grade = 0;
        grading_1 = parseFloat($("#sag_grade_1").val()) * 0.2;
        grading_2 = parseFloat($("#sag_grade_2").val()) * 0.2;
        grading_3 = parseFloat($("#sag_grade_3").val()) * 0.2;
        grading_4 = parseFloat($("#sag_grade_4").val()) * 0.4;
